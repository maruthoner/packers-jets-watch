...
// "any row" / "rows 1-10" / "row 1 only"
export function rowsPhrase({ rowMin, rowMax }) {
  if (rowMin === undefined && rowMax === undefined) return 'any row';
...
export function sectionsText(spec) {
  if (spec.groups?.length) {
    return 'sections ' + spec.groups
      .map((g) => `${(g.sections ?? []).join(', ')} (${rowsPhrase(g)})`)
      .join(' or ');
  }
...
export function decide(listings, w) {
  ...
  const located = seated.filter((l) => fitsSpec(l, w));
...
// A listing fits a list if it fits ANY of its groups — each group carrying its own
// sections and its own row rule. Without groups this is the plain section + row test.
export function fitsSpec(l, spec) {
  if (spec.groups?.length) return spec.groups.some((g) => sectionFits(l, { ...spec, ...g }) && rowFits(l, { ...spec, ...g }));
  return sectionFits(l, spec) && rowFits(l, spec);
}

// Section membership only, ignoring rows: used to decide what falls outside every list.
function coversSection(l, spec) {
  if (spec.groups?.length) return spec.groups.some((g) => sectionFits(l, { ...spec, ...g }));
  return sectionFits(l, spec);
}
...
export function decideAll(listings, w) {
  ...
  for (const spec of specs) {
    lists.push({
      id: spec.id, label: spec.label, sections: spec.sections, levels: spec.levels, groups: spec.groups, rowMax: spec.rowMax,
  ...
  const outside = specs.length ? listings.filter((l) => !specs.some((s) => coversSection(l, s))) : listings;
