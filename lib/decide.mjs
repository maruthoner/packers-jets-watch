...
// TicketWhiz builds every checkout link with "quantity=0" — its own template value,
// which it emits whatever its seat selector says (verified Sep 20 with the selector on
// 2 seats). What that does at the far end, checked live:
//   viagogo        lands on the event with the quantity filter on "Any" and the
//                  listing's own selector defaulted to 1 ticket
//   ticketnetwork  the checkout refuses outright: "we're having trouble processing
//                  your request"
// Putting the real quantity in fixes both: viagogo preselects "3 tickets", and
// TicketNetwork loads a working checkout with SelectedQuantity=3. The destination is
// URL-encoded inside the affiliate wrapper on some links and plain on others, hence
// matching both "=" and "%3D". Only the literal 0 is replaced, so a link that ever
// arrives carrying a real quantity is left alone.
export function withQuantity(url, quantity) {
  if (!url) return url;
  const q = String(Number(quantity));
  if (!/^\d+$/.test(q)) return url;
  return String(url)
    .replace(/(quantity)(=|%3D)0(?![\d])/gi, `$1$2${q}`)
    .replace(/(listingQty)(=|%3D)(?=&|$)/gi, `$1$2${q}`);
}
