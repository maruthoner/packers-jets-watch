...
// One readable line about what the page showed when it listed nothing, for the log,
// the alert and data/<id>/state.json. Diagnostics are captured only on that failure.
export function summarizeDiagnostics(d) {
  if (!d) return 'not captured';
  const arrays = d.ticketArrays?.length ? d.ticketArrays.join(', ') : 'none';
  const box = d.listBox ? `ticket list box present with ${d.listBoxRows} elements` : 'no ticket list box on the page';
  const said = d.pageText ? ` Page said: "${d.pageText.slice(0, 200)}"` : '';
  return `${box}; ticket arrays in page state: ${arrays}; status "${d.sample?.label ?? '-'}", seat selector "${d.sample?.qtyText ?? '-'}".${said}`;
}
