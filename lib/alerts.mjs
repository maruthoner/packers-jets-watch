...
function criteriaText(spec) {
  const rows = spec.groups?.length ? '' : rowsText(spec); // grouped lists carry their own
  return `${spec.quantity} seats together, ${sectionsText(spec)}${rows}, ${money(spec.priceMax)} or less each (all-in)`;
}
...
export function plan(prev, outcome, w, ctx) {
  ...
  for (const { id, spec, matches } of outcome.alerts ?? []) {
    ...
    // The issue title states the criteria, so if it has changed, the thread we were
    // using describes something we are no longer watching — and commenting into it
    // would email Ruth from a stale, usually closed, issue. Start a fresh one.
    // State written before this field existed has no title to compare, and cannot be
    // shown to match, so it is treated the same way. (Hit three times: #30, #31, #32.)
    const title = matchTitle(spec);
    if (a.issue && a.title !== title) {
      Object.assign(a, { issue: null, active: false, notifiedBest: null, emptyStreak: 0 });
    }
    a.title = title;
