...
function criteriaText(spec) {
  const where = spec.sections ? `sections ${spec.sections.join(', ')}` : 'any section';
  return `${spec.quantity} seats together, ${where}${rowsText(spec)}, ${money(spec.priceMax)} or less each (all-in)`;
}
